...
const char kWindowTitle[] = "LE2C_17_タカハシ_ユキト_MT3_02_03";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    while (Novice::ProcessMessage() == 0) {
        ...
        bool isHit = IsCollision(segment, plane, &intersection);

        // 線の描画色
        uint32_t color;

        if (isHit) {
            color = 0xFF0000FF; // 赤（交差している）
        } else {
            color = 0xFFFFFFFF; // 白（交差していない）
        }

        // 線の始点と終点をワールド→スクリーン変換して描画
