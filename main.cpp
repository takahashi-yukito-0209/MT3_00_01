...
const char kWindowTitle[] = "LE2C_17_タカハシ_ユキト_MT3_02_05";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
    ...
    while (Novice::ProcessMessage() == 0) {
        ...
        // min と max の関係を制限（min <= max）
        for (int i = 0; i < 3; ++i) {
            if (((&aabb1.min.x)[i]) > ((&aabb1.max.x)[i])) {
                (&aabb1.max.x)[i] = (&aabb1.min.x)[i];
            }
        }

        for (int i = 0; i < 3; ++i) {
            if (((&aabb2.min.x)[i]) > ((&aabb2.max.x)[i])) {
                (&aabb2.max.x)[i] = (&aabb2.min.x)[i];
            }
        }
